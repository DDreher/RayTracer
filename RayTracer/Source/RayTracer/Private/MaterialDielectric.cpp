...
{
    attenuation = colors::WHITE; // Glass absorbs nothing
    Vec3 unit_direction = glm::normalize(ray.direction_);
    ...
    float cos_theta = fminf(glm::dot(-unit_direction, hit_record.surface_normal_), 1.0f);
    ...
    if(can_refract == false || should_reflect_due_to_steep_angle)
    {
        ...
        scattered_ray_direction = glm::reflect(unit_direction, hit_record.surface_normal_);
    }
    else
    {
        scattered_ray_direction = glm::refract(unit_direction, hit_record.surface_normal_, refraction_ratio);
    }
